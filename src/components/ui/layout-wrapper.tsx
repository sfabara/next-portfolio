...
export function LayoutWrapper({ children }: LayoutWrapperProps) {
  ...
  useLayoutEffect(() => {
    ...
    cards.forEach((card, index) => {
      ...
      ScrollTrigger.create({
        trigger: card,
        start: `top ${100 + index * 120}px`, // Stagger the start positions
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <div className="absolute right-4 top-[40vh] w-[280px]">
        <div className="space-y-96"> {/* Large spacing to create scroll distance */}
          <motion.div
            ref={el => { if (el) cardsRef.current[0] = el; }}
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.6 }}
            className="bg-[#fe7500] dark:bg-[#ff3f17] rounded-2xl p-6 text-white"
          >
          ...
          <motion.div
            ...
            className="bg-[#151515] dark:bg-[#0a0a0a] rounded-2xl p-6 text-white"
          >
